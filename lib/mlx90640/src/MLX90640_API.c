...
int MLX90640_GetFrameData(uint8_t slaveAddr, uint16_t *frameData) {
  ...
  int t_start_us = get_absolute_time();
  dataReady = 0;
  while (dataReady == 0) {
    ...
    int t_end_us = get_absolute_time();

    if (t_end_us - t_start_us > 5e6) {
      return -1;
    }
  }
  ...
  for (int pixelNumber = 0; pixelNumber < 768; pixelNumber++) {
    ...
    if (pattern == frameData[833]) {
      ...
      alphaCompensated = (params->alpha[pixelNumber] -
                          params->tgc * params->cpAlpha[subPage]) *
                         (1 + params->KsTa * (ta - 25));

      Sx = pow((double)alphaCompensated, (double)3) *
