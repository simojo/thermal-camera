...
int MLX90640_SetSubPageRepeat(uint8_t slaveAddr, uint8_t subpagerepeat)
{
    ...
    value = ((uint16_t)subpagerepeat << MLX90640_CTRL_ENABLE_SUBPAGE_REPEAT_SHIFT);
    value &= ~MLX90640_CTRL_ENABLE_SUBPAGE_REPEAT_MASK;

    error = MLX90640_I2CRead(slaveAddr, MLX90640_CTRL_REG, 1, &controlRegister1);

    if(error == MLX90640_NO_ERROR)
    {
        value = (controlRegister1 & MLX90640_CTRL_ENABLE_SUBPAGE_REPEAT_MASK) | value;
